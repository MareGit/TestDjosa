           ...
           BEGIN TRY
               IF NOT UPDATE ([Status])
                   BEGIN
                       ...
                       SET    [Sales].[SalesOrderHeader].[RevisionNumber] = [Sales].[SalesOrderHeader].[RevisionNumber] + 1
                       WHERE  [Sales].[SalesOrderHeader].[SalesOrderID] IN (SELECT [inserted].[SalesOrderID]
                                                                            FROM   [inserted]);
                   END
               IF UPDATE ([SubTotal])
                   BEGIN
                       ...
                       SET    [Sales].[SalesPerson].[SalesYTD] = (SELECT SUM([Sales].[SalesOrderHeader].[SubTotal])
                       ...
                       WHERE  [Sales].[SalesPerson].[BusinessEntityID] IN (SELECT DISTINCT [inserted].[SalesPersonID]
                                                                           FROM   [inserted]
                                                                           WHERE  [inserted].[OrderDate] BETWEEN @StartDate AND @EndDate);
                       -- Update the SalesTerritory SalesYTD when SubTotal is updated
                       UPDATE [Sales].[SalesTerritory]
                       SET    [Sales].[SalesTerritory].[SalesYTD] = (SELECT SUM([Sales].[SalesOrderHeader].[SubTotal])
                                                                     FROM   [Sales].[SalesOrderHeader]
                                                                     WHERE  [Sales].[SalesTerritory].[TerritoryID] = [Sales].[SalesOrderHeader].[TerritoryID]
                                                                            -- Shipped
                                                                            AND ([Sales].[SalesOrderHeader].[Status] = 5)
                                                                            AND [Sales].[SalesOrderHeader].[OrderDate] BETWEEN @StartDate AND @EndDate)
                       WHERE  [Sales].[SalesTerritory].[TerritoryID] IN (SELECT DISTINCT [inserted].[TerritoryID]
                                                                         FROM   [inserted]
                                                                         WHERE  [inserted].[OrderDate] BETWEEN @StartDate AND @EndDate);
                   END
           END TRY
