           ...
           BEGIN TRY
               IF NOT UPDATE ([Status])
                   BEGIN
                       ...
                       SET    [Purchasing].[PurchaseOrderHeader].[RevisionNumber] = [Purchasing].[PurchaseOrderHeader].[RevisionNumber] + 1
                       WHERE  [Purchasing].[PurchaseOrderHeader].[PurchaseOrderID] IN (SELECT [inserted].[PurchaseOrderID]
                                                                                       FROM   [inserted]);
                   END
           END TRY
           BEGIN CATCH
